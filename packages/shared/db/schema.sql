-- Canonical SQLite schema (shared by Tauri desktop + Express server)
-- Desktop-only extras: see schema-desktop.sql (settings table)
...
CREATE TRIGGER IF NOT EXISTS collections_fts_insert AFTER INSERT ON collections BEGIN
    INSERT INTO collections_fts(rowid, title, content, summary, raw_content) VALUES (new.rowid, new.title, new.content, new.summary, new.raw_content);
END;
